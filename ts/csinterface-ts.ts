...
export class CSInterface {
    ...
    closeExtension(){
        (<any>window).__adobe_cep__.closeExtension();
    };
    ...
    getSystemPath(pathType: string): string{
        var path = decodeURI((<any>window).__adobe_cep__.getSystemPath(pathType));
     ...
     */
    evalScript(script: string, callback: (executionResult: any) => void){
    ...
    getHostCapabilities(): any{
        var hostCapabilities = JSON.parse((<any>window).__adobe_cep__.getHostCapabilities() );
    ...
    dispatchEvent(event: CSEvent){
        ...
        (<any>window).__adobe_cep__.dispatchEvent(event);
    };
     ...
     *         Default is null.
     */
    addEventListener(type: string, listener: any, obj?: any){
        (<any>window).__adobe_cep__.addEventListener(type, listener, obj);
    };
     ...
     *          Default is null.
     */
    removeEventListener(type: string, listener: any, obj?: any){
        (<any>window).__adobe_cep__.removeEventListener(type, listener, obj);
    };
    ...
    requestOpenExtension(extensionId: string, params: string){
        (<any>window).__adobe_cep__.requestOpenExtension(extensionId, params);
    };
    ...
    getExtensions(extensionIds: string[]): any{
        ...
        var extensionsStr = (<any>window).__adobe_cep__.getExtensions(extensionIdsStr);
    ...
    getNetworkPreferences(): any {
        var result = (<any>window).__adobe_cep__.getNetworkPreferences();
    ...
    initResourceBundle(): any {
        var resourceBundle = JSON.parse((<any>window).__adobe_cep__.initResourceBundle());
    ...
    dumpInstallationInfo(): string {
        return (<any>window).__adobe_cep__.dumpInstallationInfo();
    };
    ...
    getScaleFactor(): number {
        return (<any>window).__adobe_cep__.getScaleFactor();
    };
    ...
    getCurrentApiVersion(): any {
        var apiVersion = JSON.parse((<any>window).__adobe_cep__.getCurrentApiVersion());
        return apiVersion;
    };
     ...
     *
     */
    setPanelFlyoutMenu(menu: string) {
        if ("string" != typeof menu)
        {
            return;
        }

        (<any>window).__adobe_cep__.invokeSync("setPanelFlyoutMenu", menu);
    };
    ...
    updatePanelMenuItem(menuItemLabel: string, enabled: boolean, checked: boolean): boolean {
        ...
        if (this.getHostCapabilities().EXTENDED_PANEL_MENU)
        {
            ...
            ret = (<any>window).__adobe_cep__.invokeSync("updatePanelMenuItem", JSON.stringify(itemStatus));
        }
        return ret;
    };
     ...
     * </Menu>
     */
    setContextMenu(menu: string, callback: (clikedMenuId: string) => {}) {
        if ("string" != typeof menu)
        {
            return;
        }

        (<any>window).__adobe_cep__.invokeAsync("setContextMenu", menu, callback);
    };
     ...
     *
     */
    setContextMenuByJSON(menu: string, callback: (clikedMenuId: string) => {}) {
        if ("string" != typeof menu)
        {
            return;
        }

        (<any>window).__adobe_cep__.invokeAsync("setContextMenuByJSON", menu, callback);
    };
     ...
     * @param checked        True to select the item, false to deselect it.
     */
    updateContextMenuItem(menuItemID: string, enabled: boolean, checked: boolean) {
        var itemStatus = new ContextMenuItemStatus(menuItemID, enabled, checked);
        ret = (<any>window).__adobe_cep__.invokeSync("updateContextMenuItem", JSON.stringify(itemStatus));
    };
    ...
    isWindowVisible(): boolean {
        return (<any>window).__adobe_cep__.invokeSync("isWindowVisible", "");
    };
    ...
    resizeContent(width: number, height: number) {
        (<any>window).__adobe_cep__.resizeContent(width, height);
    };
    ...
    registerKeyEventsInterest(keyEventsInterest: string): any {
        return (<any>window).__adobe_cep__.registerKeyEventsInterest(keyEventsInterest);
    };
    ...
    setWindowTitle(title: string) {
        (<any>window).__adobe_cep__.invokeSync("setWindowTitle", title);
    };
    ...
    getWindowTitle(): string {
        return (<any>window).__adobe_cep__.invokeSync("getWindowTitle", "");
    };
}
...
export class CSEvent {
     ...
     */
    constructor(public type: string, public scope: string, public appId: string, public extensionId: string) {
    }
...
export class GradientColor {
     ...
     */
    constructor(public type: string, public direction: Direction, public numStops: number, public arrGradientStop: GradientStop[]) {
    }
}
...
export class AppSkinInfo {
     ...
     */
    constructor(public baseFontFamily: string,
...
export class HostEnvironment {
     ...
     */
    constructor(public appName: string, public appVersion: any, public appLocale: any, public appUILocale: any, public appId: string, public isAppOnline: boolean, public appSkinInfo: AppSkinInfo){
...
export class HostCapabilities {
     ...
     */
    constructor(public EXTENDED_PANEL_MENU: boolean,
...
export class ApiVersion {
     ...
     */
    constructor(public major: number, public minor: number, public micro: number) {
